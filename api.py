...
def add_member():
    ...
    return jsonify(
        {'id': new_member['id'], 'name': new_member['name'], 'email': new_member['email'],
...
def edit_member(member_id):
    ...
    db = get_db()
    db.execute('update customers set name =?, email = ?, level = ? where id= ?', [name, email, level, member_id])
    db.commit()
...
def delete_member(member_id):
    db = get_db()
    db.execute('delete from customers where id = ?', [member_id])
    db.commit()
